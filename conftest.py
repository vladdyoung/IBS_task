...
def pytest_addoption(parser):
    parser.addoption(
        ...
        help="Enter name of browser for testing: chrome, firefox or remote"
    )
    parser.addoption(
        "--browser_name_remote",
        default="chrome",
    ...
    parser.addoption("--executor", default="192.168.0.180")
...
def browser(request, logger):
    ...
    if browser_name == "chrome":
        ...
        latest_chrome_version_json = requests.get(f"http://{executor}:8080/status").json().get("state").get("browsers")
        latest_chrome_version = list(latest_chrome_version_json.get(browser_name_remote).keys())[0]
        capabilities = {
            "browserName": browser_name_remote,
            "browserVersion": latest_chrome_version,
            "selenoid:options": {
                "enableVNC": vnc,
                "enableVideo": False
            }
...
def pytest_sessionstart():
    ...
    if os.path.isdir(os.path.dirname(__file__) + "logs"):
        shutil.rmtree(os.path.dirname(__file__) + "logs")
